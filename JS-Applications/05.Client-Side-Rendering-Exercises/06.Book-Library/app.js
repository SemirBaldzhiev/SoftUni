...
render(bookLibaryTemplate([], forms, loadBooks, prepareEdit, prepareDelete), body);
...
async function loadBooks() {
    ...
    console.log(books);
    render(allBooksTemplate(books, prepareEdit, prepareDelete), booksContainer);
}
...
async function prepareEdit(e){
    ...
    render(bookLibaryTemplate(books, forms, loadBooks, prepareEdit, prepareDelete), body);
}
...
async function prepareDelete(e){
    let book = e.target.closest('.book');
    let id = book.dataset.id;
    let bookToDelete = await booksService.deleteBook(id);
    books = books.filter(b => b.title !== bookToDelete.title)
    console.log(books);
    render(allBooksTemplate(books, prepareEdit, prepareDelete), booksContainer);
}
